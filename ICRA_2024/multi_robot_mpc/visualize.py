...
class Animation:
  def __init__(self, filename_env, filename_result = None, filename_output = None, filename_expanded_trajs = None):
  ...
  def animate_func(self, i):
    ...
    for k, robot in enumerate(self.result["result"]): # for each robot
      if i >= len(robot["states"]):
        ...
        if self.robot_types[k] == 'integrator1_2d_v0':
            pos = state
            xy = np.asarray(pos)
        ...
        elif self.robot_types[k] == 'double_integrator_2d':
            pos = state[:2]
            xy = np.asarray(pos)
            self.robot_patches[k][0].center = xy
            t = matplotlib.transforms.Affine2D().rotate_around(
        ...
        elif self.robot_types[k] == 'unicycle_first_order' or self.robot_types[k] == 'car_first_order_0' or self.robot_types[k] == 'unicycle2_v0':
            pos = state[:2]
            yaw = state[2]
    ...
    elements.extend([item for row in self.robot_patches for item in row])
    return elements

  def draw_robot(self, state, type, **kwargs):
    patches = []
    if type == 'integrator1_2d_v0':
      pos = state
      patches.append(draw_sphere_patch(self.ax, state, self.radius, 0, **kwargs))
    elif type == 'double_integrator_2d':
        pos = state[:2]
        patches.append(draw_sphere_patch(self.ax, state, self.radius, 0, **kwargs))
    ...
    elif type == 'unicycle_first_order' or type == 'car_first_order_0' or type == 'unicycle2_v0':
        pos = state[:2]
        yaw = state[2]
        pos2 = pos + np.array([np.cos(yaw), np.sin(yaw)])*self.size[0]/2*0.8
...
def visualize(filename_env, filename_result = None, filename_video=None, filename_expanded_trajs=None):
  anim = Animation(filename_env, filename_result, filename_video, filename_expanded_trajs)
...
def main():
  ...
  parser.add_argument("--trajs", help="output file containing expanded trajs")
  args = parser.parse_args()
  visualize(args.env, args.result, args.video, args.trajs)
